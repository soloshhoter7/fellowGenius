...
//	    props.put("mail.smtp.socketFactory.fallback", "false");  
	...
	void sendRequestToReschedule(BookingDetails booking) {
		...
		try {
			MimeMessage message = new MimeMessage(session);
				...
				message.setFrom(new InternetAddress(senderEmail));
				message.addRecipient(Message.RecipientType.TO, new InternetAddress(to));
				message.setSubject("Reschedule the appointment !");
				message.setContent("<html><head> <link rel=\"stylesheet\" href=\"https://maxcdn.bootstrapcdn.com/bootstrap/4.4.1/css/bootstrap.min.css\" /> <style> .Box { box-shadow: 0 8px 16px 0 #90a4ae; width: 100%; height: auto; text-align: center; padding: 20px; margin-top: 10px; background-size: contain; border: 1px solid #7d0f7d; border-radius: 8px; width: 650px; margin: 0 auto; background-position: left top; } .logo { background: url(https://fellowgenius.com/logo.2dbc598173218fe92921.svg) no-repeat; background-size: contain; height: 100px; display: block; float: right; width: 100px; } tr { padding: 10px; } td { padding: 5px; margin-right: 5px; } </style></head><body> <div class=\"Box\"> <div class=\"box-header\"> <span class=\"logo\"></span> <span style=\"margin-bottom: 20px;color:#892687;width:100%;text-align: center;font-size:16px;font-weight:bold;text-transform: uppercase;\">Reschedule the appointment</span> </div> <div style=\"padding:20px;width: 350px; margin: 0 auto;\"> <p>Expert has requested to reschedule the following meeting due to some reason</p> "
						+ "<table> <tr> <td><b>Expert Name</b><br>"+booking.getTutorName()+"</td> "
						+ "<td><b>Date</b><br>"+booking.getDateOfMeeting()+"</td> </tr>"
						+ " <tr> <td><b>Start Time</b><br>"+booking.getStartTimeHour() + ":"+ booking.getStartTimeMinute() +"</td> "
						+ "<td><b>Duration</b><br>"+booking.getDuration()+" minutes</td> </tr> </table> </div> <p>As a learner, you have two options to proceed either you can cancel the meeting or reschedule the meeting with same tutor </p> <div style=\"padding:20px;width: 350px; margin: 0 auto;\"> <p>Please click on the link mentioned below to reschedule the meeting</p> "
						+ "<a href=\""+rescheduleUrl+"\"class=\\\"btn btn-primary\\\">Reschedule meeting</a> </div> <div style=\"padding:20px;width: 350px; margin: 0 auto;\"> <p>Please click on the link mentioned below to cancel the meeting</p> "
						+ "<a href=\""+cancelUrl+"\"class=\\\\\\\"btn btn-primary\\\\\\\">Cancel meeting</a> </div> <p style=\"margin-top: 30px; font-size: 10px; text-align: center; width: 100%;\">This is an auto-generated message please don't reply back.</p> </div> <div class=\"container-fluid\"> <div class=\"row\"> <div class=\"col-sm-2\"></div> <div class=\"col-sm-7\"> </div> <div class=\"com-sm-3\"></div> </div> </div></body></html>","text/html");
				Transport.send(message);
	...
	boolean sendResetMail(String email) {
		...
		if (user != null) {
			...
			String directUrl = rootUrl+"resetPassword?token="+token;
