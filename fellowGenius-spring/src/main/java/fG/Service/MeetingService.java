...
//		    LocalDateTime then = now.minusDays(1);
		    ...
		    for(int i=1;i<=7;i++) {
		    	...
		    	List<BookingDetails> bk = repBooking.fetchExpertMeetingsBetweenTwoDates(exp.getBookingId(),then, now.minusDays(i-1));
		    ...
		    for(int i=1;i<=6;i++) {
		    	...
		    	List<BookingDetails> bk = repBooking.fetchExpertMeetingsBetweenTwoDates(exp.getBookingId(),then, now.minusMonths(i-1));
		    	if(bk!=null) {
		    		for(BookingDetails booking:bk) {
			    		if(!booking.getApprovalStatus().equals("cancelled")) {
			    			if(!uniqueLearner.contains(booking.getStudentId())) {
		    ...
		    for(int i=1;i<=6;i++) {
		    	...
		    	List<BookingDetails> bk = repBooking.fetchExpertMeetingsBetweenTwoDates(exp.getBookingId(),then, now.minusYears(i-1));
		    	if(bk!=null) {
		    		for(BookingDetails booking:bk) {
			    		if(!booking.getApprovalStatus().equals("cancelled")) {
			    			if(!uniqueLearner.contains(booking.getStudentId())) {
