...
$(async () => {
  ...
  $(".cam-list").delegate("a", "click", function(e){
    this.switchCamera(this.text);
  });
  $(".mic-list").delegate("a", "click", function(e){
    this.switchMicrophone(this.text);
  });
...
$("#media-device-test").on("hidden.bs.modal", function (e) {
  ...
  if (this.options.appid && this.options.channel) {
    $("#appid").val(this.options.appid);
    $("#token").val(this.options.token);
    $("#channel").val(this.options.channel);
    $("#join-form").submit();
  }
})
  }
  ...
  async formJoinSubmit(e){
    ...
    try {
      ...
      if(this.options.token) {
        ...
        $("#success-alert a").attr("href", `index.html?appid=${this.options.appid}&channel=${this.options.channel}&token=${this.options.token}`);
  ...
  async join() {
    ...
    if (!this.localTracks.audioTrack || !this.localTracks.videoTrack) {
      [ this.localTracks.audioTrack, this.localTracks.videoTrack ] = await Promise.all([
        ...
        AgoraRTC.createMicrophoneAudioTrack({ microphoneId: this.currentMic.deviceId }),
        AgoraRTC.createCameraVideoTrack({ cameraId: this.currentCam.deviceId })
      ]);
    }
