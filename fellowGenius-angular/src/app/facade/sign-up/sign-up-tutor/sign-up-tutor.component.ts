...
export class SignUpTutorComponent implements OnInit {
  ...
  emailPattern =
    "[a-zA-Z0-9_.+-]{1,20}@(?:(?:[a-zA-Z0-9-]+.)?[a-zA-Z]+.)?(gmail|piet|yahoo|hotmail|rediffmail|apple|microsoft).([a-z]{2,4})(.[a-z]{2,4})?$";
  userId;
      ...
      (response: any) => {
        if (response.authResponse) {
          ...
          FB.api("/me?fields=name,email,picture", (res: any) => {
            ...
            this.socialLogin.fullName = res.name;
            this.httpClient
              .saveSocialLogin(this.socialLogin)
              .subscribe((res) => {
                if (res) {
                  var loginModel = new tutorLoginModel();
                  loginModel.email = this.socialLogin.email;
                    ...
                    .subscribe((res) => {
                      if (res["response"] != "false") {
                        ...
                        this.httpClient
                          ...
                          .subscribe((res) => {
                            this.tutorProfile = res;
                            this.tutorService.setTutorDetails(
                              this.tutorProfile
                            );
