...
export class tutorScheduleComponent implements OnInit {
	...
	copySchedule() {
		for (let schedule of this.tutorService.getPersonalAvailabilitySchedule().allAvailabilitySchedule) {
	...
	onActionBegin(args: ActionEventArgs): void {
		...
		if (args.requestType === 'eventCreate') {
			...
			setTimeout(() => {
				if (schedule[0].Guid != null && schedule[0].RecurrenceException != null) {
