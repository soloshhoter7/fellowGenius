...
export class MeetingComponent implements OnInit {
	...
	captureEvents(canvasEl: HTMLCanvasElement) {
		...
		fromEvent(canvasEl, 'mousedown')
			...
			.subscribe((res: [MouseEvent, MouseEvent]) => {
				...
				this.sendCoordinates('drawing', null, this.cx.strokeStyle, this.cx.lineWidth, prevPos, currentPos);
				let coordinates = {
					prevX: prevPos.x,
					prevY: prevPos.y,
					currX: currentPos.x,
	...
	connectToMeetingWebSocket(bookingId) {
		...
		this.ws.connect(
			...
			(frame) => {
				...
				that.ws.subscribe('/inbox/MeetingChat/' + bookingId, function(message) {
	...
	openBlob(message: MessageModel) {
		...
		message.dataSource = this.sanitizer.bypassSecurityTrustUrl(URL.createObjectURL(file));
	}
