...
export class TutorDashboardComponent implements OnInit {
  ...
  intialiseCompletedMeetings() {
    console.log("Inside completed meetings function");
    this.tutorService.fetchTutorCompletedMeetings();
    this.tutorService.completedMeetingsChanged.subscribe(
      (booking: bookingDetails[]) => {
        this.previousMeetingList = booking;
        console.log("Inside previous meeting list");
        console.log(this.previousMeetingList);
        if (this.previousMeetingList.length != 0) {
          this.sortMeetings(this.previousMeetingList);
        } else {
