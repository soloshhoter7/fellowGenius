...
export class SearchResultsComponent implements OnInit {
	...
	searchByPrice($event) {
		if (this.checkFiltersApplied(this.priceFiltersApplied, $event.target.value)) {
			...
			for (var i = this.filteredArray.length - 1; i >= 0; i--) {
				if (Number(this.filteredArray[i].price1) >= Number(lowerValue) && Number(this.filteredArray[i].price1) <= Number(higherValue)) {
					this.filteredArray.splice(i, 1);
				}
			}
			this.priceFiltersApplied.splice(this.priceFiltersApplied.indexOf($event.target.value), 1);
			this.allFiltersApplied.splice(this.allFiltersApplied.indexOf($event.target.value), 1);
		} else {
	...
	searchByRatings($event){
		...
		if (this.checkFiltersApplied(this.ratingFilterApplied, $event.target.value)) {
			...
			if(this.ratingFilterApplied.length==1){
				...
				if(status){
					this.ratingFilterApplied.splice(this.ratingFilterApplied.indexOf($event.target.value), 1);
					this.allFiltersApplied.splice(this.allFiltersApplied.indexOf($event.target.value), 1);
				}else{
					this.ratingFilterApplied.sort();
					var smallest = Math.min.apply(Math, this.ratingFilterApplied);
					var nextToSmallest = this.ratingFilterApplied[this.ratingFilterApplied.indexOf(smallest)+2];
					
			for (var i = this.filteredArray.length - 1; i >= 0; i--) {
				if (this.filteredArray[i].rating >= 20*smallest && this.filteredArray[i].rating <20* nextToSmallest) {
					this.filteredArray.splice(i, 1);
				}
			}
			this.ratingFilterApplied.splice(this.ratingFilterApplied.indexOf($event.target.value), 1);
	...
	searchBySubject($event) {
		...
		if (this.checkFiltersApplied(this.subjectFiltersApplied, $event.target.value)) {
			if(this.subjectFiltersApplied.length==1){
				...
				for (var i = this.filteredArray.length - 1; i >= 0; i--) {
					if(this.filteredArray[i].areaOfExpertise.includes($event.target.value)){
						if(this.removeOrNot(this.filteredArray[i].areaOfExpertise, this.subjectFiltersApplied, $event.target.value)){
							this.filteredArray.splice(i,1);
						}
					}
						
				}
				this.subjectFiltersApplied.splice(this.subjectFiltersApplied.indexOf($event.target.value), 1);
				this.allFiltersApplied.splice(this.allFiltersApplied.indexOf($event.target.value), 1)
	...
	checkDuplicacyOfObjectInArray(array, object) {
		if (array.filter(e => JSON.stringify(e) === JSON.stringify(object)).length > 0) {
			return true;
		}
	}
